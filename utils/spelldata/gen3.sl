...
sub process
{
   ...
   while $entry (getthree(@words))
   {
      ...
      if (%words[$current] !is $null && %dictionary[$previous] !is $null && %dictionary[$next] !is $null && %counts[$current] < 10)
      {
         println($output, "$previous * $next $+ |" . join(", ", concat($current, %dataset[$current])) );
         %counts[$current] += 1;
      }
   }
}
